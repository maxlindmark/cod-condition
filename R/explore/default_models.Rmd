...
We can also plot the residuals by year and size (indicative of a change in the length-coefficient over time)
...
We can though fit a model with a time-varying (random walk) length-exponent and see if 1) there is a trend in the estimates and 2) it "improves" the residuals plots. Note that we below fit year as a factor and have only the length coefficient as a random walk:

```{r time-varying length-coefficient and factor year, fig.width=12, message=FALSE, cache=TRUE}
# This is a modification of model m2, where instead of a rw-intercept have year as a factor, and include a time-varying length-coefficient instead
m2b <- sdmTMB(formula = ln_weight_g ~ factor(year) -1, time_varying = ~ 0 + ln_length_cm, data = d, time = "year",
              spde = bspde, family = student(link = "identity", df = 5), ar1_fields = TRUE,
              include_spatial = TRUE, spatial_trend = FALSE, spatial_only = FALSE,
              silent = TRUE, newton_steps = 1, reml = FALSE)
```

And now explore this model:

```{r rw length coefficients, fig.width=12}
# Extract and plot the length-exponents over year
sdm2b <- as.data.frame(summary(TMB::sdreport(m2b$tmb_obj)))

sdm2b <- sdm2b %>% 
  rownames_to_column("param") %>%
  filter(stringr::str_detect(param, 'b_rw_t')) %>% 
  rename("s.e" = "Std. Error") %>% 
  mutate(year = sort(unique(d$year)),
         upr = Estimate + 1.96*s.e,
         lwr = Estimate - 1.96*s.e)
  
ggplot(sdm2b, aes(year, Estimate)) +
  geom_point(size = 3) +
  ylab("Length-exponent") + 
  geom_errorbar(aes(x = year, ymax = upr, ymin = lwr),
                width = 0.2, alpha = 0.8) + 
  theme(axis.text.x = element_text(angle = 30),
        legend.position = c(0.8, 0.8)) +
...
Ok, so they vary across time. Does it "improve" residuals?

```{r rw length coefficients resid, fig.width=12}
# We can also look at the residuals in detail and not stare too much on the mean gam fit
df$residuals_m2b <- residuals(m2b)

ggplot(df, aes(ln_length_cm, residuals_m2b)) +
  geom_point(alpha = 0.5, color = "grey20") + 
  geom_abline(color = "red", slope = 0, linetype = 2) +
  geom_smooth(data = df, aes(ln_length_cm, residuals_m2, group = year, color = factor(year)),
              inherit.aes = FALSE) + 
...
So, maybe a random walk on the residuals does not improve the residuals, and maybe they are not even problematic in the first place. But there is a decline in the length exponent over time, and this could lead to biases in the condition for the same reason that we do not use Fulton's K! Therefore, I will proceed with the random walk model for length at year as a factor.

We can next plot the predictions from the random walk on the length-coefficients, because it might lead to different predictions than the model with a constant length-exponent through time.

```{r test predict on grid, message=FALSE,fig.width=12}
# Predict using the random walk on length model:
p_rw_length <- predict(m2b, newdata = pred_grid)

# Predict using the random walk on year model:
p_rw_year <- predict(m2, newdata = pred_grid)

# Replace too-deep predictions with NA
p_rw_length <- p_rw_length %>% mutate(est2 = ifelse(depth > 120, NA, est)) # prediction (fixed + random)
p_rw_year <- p_rw_year %>% mutate(est2 = ifelse(depth > 120, NA, est)) # prediction (fixed + random)

# Plot rw length
ggplot(p_rw_length, aes(lon, lat, fill = est2)) +
  geom_raster() +
  facet_wrap(~year, ncol = 5) +
  ...
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) + 
  ggtitle("Prediction (rw length)")

# Plot rw year
ggplot(p_rw_year, aes(lon, lat, fill = est2)) +
  geom_raster() +
  facet_wrap(~year, ncol = 5) +
  scale_fill_viridis(option = "magma", 
                     name = "log(condition factor)") + 
  geom_sf(data = world, inherit.aes = F, size = 0.2) +
  coord_sf(xlim = c(xmin, xmax), ylim = c(ymin, ymax)) + 
  ggtitle("Prediction (rw year)")
```

Ok, this seems strange because the difference is very large. In fact, the overall decline in the random walk length model is almost blurred by the large interannual changes. And this actually makes sense! Because we assume the whole population is all cod in all years across the spatial area. So when we let the exponent vary across years, then we remove the trend we want to look at by letting the predicted LW-relationship be different each year (have its own exponent). This has an effect of the predictions for the condition factor, even though we predict at 0 length!

So it must affect the intercepts, and we can show that here:

```{r annual intercept from rw length vs year,cache=TRUE,message=FALSE}
rw_year <- data.frame(summary(TMB::sdreport(m2$tmb_obj))) %>% 
  rownames_to_column("param") %>%
  filter(stringr::str_detect(param, 'b_rw_t')) %>% 
  rename("se" = "Std..Error") %>% 
  mutate(year = sort(unique(d$year)),
...
rw_length <- data.frame(summary(TMB::sdreport(m2b$tmb_obj))) %>% 
  rownames_to_column("param") %>%
  filter(stringr::str_detect(param, 'b_j')) %>% # These are the intercepts, the length-coefficient is simply b_j
  rename("se" = "Std..Error") %>% 
  mutate(year = sort(unique(d$year)),
         upr = Estimate + 1.96*se,
         lwr = Estimate - 1.96*se,
...
So actually we should instead try and find the best length-exponent across the whole time period and look at the condition factor given that overall exponent... 

Before proceeding though, we can also check the residuals on a map:
...
m2b_est <- tidy(m2b, effects = "ran_par", conf.int = TRUE) %>%
  filter(term %in% c("sigma_O", "sigma_E")) %>% 
  mutate(model = "default model no depth")

coef_df <- bind_rows(moxy_est, m2c_est, m2b_est) 
dodge <- position_dodge(width = 0.3)
