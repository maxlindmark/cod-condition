...
$$
\alpha_t \sim \operatorname{Normal}(\gamma_{t-1}, \sigma^2_\gamma).
$$
...
tidy(m2, effects = "ran_pars", confint = TRUE)
# Transform back to -1 to 1 scale
2 * plogis(tidy(m2, effects = "ran_pars", confint = TRUE)$estimate[5]) - 1
2 * plogis(tidy(m2, effects = "ran_pars", confint = TRUE)$estimate[5] + c(-2, 2) * tidy(m2, effects = "ran_pars", confint = TRUE)$std.error[5]) - 1
```
...
p <- p %>% mutate(est2 = ifelse(depth < -130, NA, est), # prediction (fixed + random)
                  ...
                  est_non_rf2 = ifelse(depth < -130, NA, est_non_rf)) # spatial trend
```
