#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
# 2020.09.30: Max Lindmark
#
# In this script I read in CTD data from ICES and match by the nearest points in space
# However, this leads to some distances between hauls and ctd samples being large
# filtering by a maximum distance of 5km removes almost 60% of the condition data.
# Hence I will instead attempt to fit a spatiotemporal model.
# 
#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
# E. READ AND JOIN THE CPUE COVARIATES =============================================
...
# Check if unique!
# It is with the except of 8 rows. Not much I can do about that because I don't have
...
oxy2 <- oxy %>% 
  ...
  separate(yyyy.mm.ddThh.mm_temp, c("year", "temp"), sep = 4) %>% 
  separate(temp, c("temp", "month"), sep = c(1,3)) %>% 
  mutate(month = as.numeric(month)) %>% 
  mutate(quarter = ifelse(month < 4, 1, 2),
  ...
  dplyr::select(-kattegatt) %>% 
  filter(quarter == 4) %>% 
  mutate(ID4 = paste(year, lat, lon, sep = "_")) %>% 
  filter(!DOXY..ml.l. %in% c("<0.00", "<0.01", "<0.02", "<0.10")) %>% # remove these characters
  mutate(DOXY..ml.l._num = as.numeric(DOXY..ml.l.))

# ggplot(oxy2, aes(factor(quarter), fill = factor(month))) + geom_bar()

# How many datapoints do I have per unique combination of lat-lon?
oxy2 %>% 
  group_by(ID4) %>% 
  mutate(n = n()) %>% 
...
# Since there are multiple samples per date and lon-lat, I will summarize by ID4
# (i.e. Year, Lat and Lon)
oxy3 <- oxy2 %>% 
  group_by(ID4) %>% 
  summarise(mean_DOXY..ml.l._num = mean(DOXY..ml.l._num)) %>% 
  mutate(ID4_temp = ID4) %>% 
  separate(ID4_temp, c("Year", "lat", "lon"), sep = "([_])") %>% 
  mutate_at(c("Year", "lat", "lon"), as.numeric)

ggplot(oxy3, aes(lat)) + geom_histogram()
ggplot(oxy3, aes(lon)) + geom_histogram()
...
# Looks like enough coverage in Q4... I will next join in the closet CTD measurement 
# with the haul, and then plot the distances. Then I will either remove data where the
# distance is too long (see the MSC thesis), or see if I can use data from another 
# quarter
...
# Option 2
# First create datasubsets for testing
test_dat <- dat %>%
  rename("lat" = "ShootLat",
         "lon" = "ShootLong") %>% 
  filter(Year == 2000) %>% 
  dplyr::select(lat, lon, ID4)

test_oxy3 <- oxy3 %>%
  filter(Year == 2000) %>%
  dplyr::select(lat, lon, mean_DOXY..ml.l._num) %>%
  as.data.frame()

str(test_dat)
str(test_oxy3)

d <- pointDistance(test_dat[, 2:1], test_oxy3[, 2:1], lonlat = TRUE, allpairs = FALSE)
i <- apply(d, 1, which.min)

test_dat$mean_DOXY..ml.l._num = test_oxy3$mean_DOXY..ml.l._num[i]

test_dat$distance = d[cbind(1:nrow(d), i)]
...
# Illustrate
plot(test_dat[, 2:1], col = "blue", pch = 20)
points(test_oxy3[, 2:1], col = "red", pch = 20)

for (i in 1:(nrow(test_dat))) {
  j <- test_oxy3$mean_DOXY..ml.l._num == test_dat$mean_DOXY..ml.l._num[i]
  ...
  x1 <- test_oxy3[j, 2][which.min(abs(x0 - test_oxy3[j, 2]))] 
  y1 <- test_oxy3[j, 1][which.min(abs(x0 - test_oxy3[j, 2]))] 
  
  arrows(x0 = x0, y0 = y0, x1 = x1, y1 = y1, length = .1)
}


# Now do it for the full data 
dat_cond <- dat %>%
  rename("lat" = "ShootLat",
         "lon" = "ShootLong") %>% 
...
dat_oxy <- oxy3 %>%
  dplyr::select(Year, lat, lon, mean_DOXY..ml.l._num) %>%
  # filter(Year %in% c("1991", "2000")) %>%
  # filter(Year == "2000") %>% 
  as.data.frame()
...
for (i in 1:length(unique(dat_cond$Year))) {
  ...
  temp_cond$mean_DOXY..ml.l._num = temp_oxy$mean_DOXY..ml.l._num[ii]
  ...
  plot(temp_cond[, 3:2], col = "blue", pch = 20)
  points(temp_oxy[, 3:2], col = "red", pch = 20)
  
  for (j in 1:(nrow(temp_cond))) {
    k <- temp_oxy$mean_DOXY..ml.l._num == temp_cond$mean_DOXY..ml.l._num[j]
...
# The warnings are from the arrows being too short (too close) and hence not plotted
# properly
...
data_oxy_cond <- data_oxy_cond %>%
  dplyr::select(mean_DOXY..ml.l._num, ID4, distance) %>% 
...
test <- dat %>% filter(distance < 10000)

ggplot(test, aes(distance)) +
  geom_histogram()

# Double check coordinate units!
# Add in depth and check depth diff is not too bog (else maybe model)
