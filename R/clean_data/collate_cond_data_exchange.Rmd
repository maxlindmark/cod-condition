...
# qwraps2::lazyload_cache_dir(path = "R/clean_data/collate_cond_data_exchange_cache/html")
```
...
# Cod model
mcod <- sdmTMB(density_cod ~ 0 + as.factor(year) + s(depth_sc) + s(oxy_sc) + s(temp_sc),
...
mfle <- sdmTMB(density_fle ~ 0 + as.factor(year) + s(depth_sc) + s(oxy_sc) + s(temp_sc),
               data = d, spde = spde, family = tweedie(link = "log"),
               fields = "AR1", include_spatial = TRUE, time = "year",
               spatial_only = FALSE, reml = FALSE,
               control = sdmTMBcontrol(newton_steps = 1))
...
spr <- read_xlsx("data/BIAS/abundances_rectangles_1991-2019.xlsx",
                 sheet = 1) %>%
  mutate(SD = ifelse(SD == "28_2", "28", SD)) %>% 
  rename("StatRec" = "Rec") %>%
...
abun_spr_sd <- read_xlsx("data/BIAS/abundance_sub_division_1991-2019.xlsx",
                         sheet = 1) %>%
  rename("year" = "ANNUS",
         "SD" = "Sub_Div") %>% 
  mutate(SD = ifelse(SD == "28_2", "28", SD)) %>% 
  mutate(Species = "Sprat",
         abun_spr_sd = `AGE1`+`AGE2`+`AGE3`+`AGE4`+`AGE5`+`AGE6`+`AGE7`+`AGE8+`, # omitting `0+` here
         ID_sd_year = paste(SD, year, sep = ".")) %>% # Make new ID)
  filter(SD %in% unique(dat$SD)) %>% 
  dplyr::select(abun_spr_sd, ID_sd_year)

abun_her_sd <- read_xlsx("data/BIAS/abundance_sub_division_1991-2019.xlsx",
                         sheet = 2) %>%
  rename("year" = "ANNUS",
         "SD" = "Sub_Div") %>% 
  mutate(SD = ifelse(SD == "28_2", "28", SD)) %>% 
  mutate(Species = "Herring",
         abun_her_sd = `AGE1`+`AGE2`+`AGE3`+`AGE4`+`AGE5`+`AGE6`+`AGE7`+`AGE8+`, # omitting `0+` here
         ID_sd_year = paste(SD, year, sep = ".")) %>% # Make new ID)
  filter(SD %in% unique(dat$SD)) %>% 
  dplyr::select(abun_her_sd, ID_sd_year) 

dat <- dat %>% mutate(ID_sd_year = paste(SD, year, sep = "."))

dat <- left_join(dat, abun_spr_sd)

dat <- left_join(dat, abun_her_sd)

head(data.frame(dat))

dat %>% 
  group_by(year, SD) %>% 
  summarise(sum_her_across_rect = sum(abun_her)) %>% 
  ungroup() %>% 
  ggplot(.) + 
  geom_point(aes(year, sum_her_across_rect, color = "rec")) +
  geom_point(data = dat, aes(year, abun_her_sd*100, color = "sd")) + 
  facet_wrap(~ SD, scales = "free", ncol = 2)

dat %>% 
  ggplot(.) + 
  geom_point(data = dat, aes(year, abun_her_sd, color = "sd")) + 
  facet_wrap(~ SD, scales = "free", ncol = 2)
...
dat$biomass_saduria <- extract(saduria_longlat, dat[, 5:4])
```
