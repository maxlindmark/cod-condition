...
# average prediction on those levels. To achieve that, I will join in those averages from the 
# pred_grid:
...
dat <- dat %>%
  dplyr::select(-IDx, -quarter, -IDx, -year_sd_id, -year_rect_id) %>% 
  dplyr::select(-density_cod_data, -density_fle_data) %>% 
  drop_na()
