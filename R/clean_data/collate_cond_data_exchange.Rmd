...
dat$depth <- extract(dep_rast, dat %>% dplyr::select(lon, lat), method = "bilinear")
...
dat$density_saduria <- extract(saduria_longlat, dat %>% dplyr::select(lon, lat), method = "bilinear")
...
pred_grid_sd_vars <- pred_grid %>% 
  ...
  dplyr::select(year_sd_id, biomass_spr_sd, biomass_her_sd)
...
# density_cod_sc         -0.0037685  0.0005036   -7.484 7.29e-14 ***
# density_fle_sc          0.0018239  0.0005206    3.503  0.00046 ***
...
#                         Estimate   Std. Error  t value Pr(>|t|)    
# density_cod_data_sc    -0.0018478  0.0004664   -3.962 7.45e-05 ***
# density_fle_data_sc     0.0011130  0.0004946    2.250  0.02444 *  

dd <- data.frame(est = c(-0.0037685, 0.0018239, -0.0018478, 0.0011130),
                 se = c(0.0005036,  0.0005206, 0.0004664, 0.0004946),
                 source = rep(c("model", "data"), each = 2),
                 term = rep(c("cod", "flounder"), 2)) %>% 
  mutate(lwr = est - 2*se,
         upr = est + 2*se)
  
ggplot(dd, aes(term, est, ymin = lwr, ymax = upr, color = source)) + 
