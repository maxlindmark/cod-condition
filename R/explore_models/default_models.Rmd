...
$$
\operatorname{log}(w_{s,t}) = \alpha_0 + \alpha_mS_m + \alpha_uS_u + \omega_s + \epsilon_{s,t} + \sum^{n_k}_{k=1}\boldsymbol\gamma_k\boldsymbol{X_k} + \beta \operatorname{log}(l)
$$ 
...
ML: Right! Forgot about that, I did see your new model description which is great
$$
\boldsymbol\omega \sim \operatorname{MVNormal}(\boldsymbol0, \boldsymbol\Sigma_\omega)\\
\boldsymbol\epsilon \sim \operatorname{MVNormal}(\boldsymbol0, \boldsymbol\Sigma_\epsilon).
$$
...
I aim to find an appropriate model structure for Eq. 1, to which I later can include and evaluate additional covariates ($\gamma_k\boldsymbol{X_k}$). To find this default model, I fit a Student-t model (to account for the presence of heavy tails.) with only length as a covariate with different random effects (fitted with `reml = TRUE`). Then I will fit the selected model with different fixed effects: year and sex using `reml = FALSE`.

The different models are:
* Time-varying intercept through random walk
* With spatial trend (random field representing a slope over the time variable)
* Autoregressive spatiotemporal field 
...
ML: Interesting options! I will explore them separately I think, can report back if you'd like!
...
ML: Perfect! I assume use a Student-t distribution with 5 degrees of freedom (initial exploration suggested the default df=3 was too heavy-tailed).

**To reiterate**: first we select the random structure by fitting the following models with `reml = TRUE`

1) Time-varying intercept through random walk

2) With spatial trend (random field representing a slope over the time variable)

3) Autoregressive spatiotemporal field 


Followed by fitting models with the following fixed effects and `reml = FALSE`:
1) Fixed effect of year

2) Fixed effect of year
...
# Ok! I think with a glm the predicted weight would be exp(a + b*length), so the length-coefficient would not be the allometric exponent b. Log-transforming is pretty standard (Froese & Thorson, 2014), but some argue for non-linear models also (e.g. papers by Packard). I think Thorson (2015) uses an identity link, but Grüss et al (2020) use a log link function... but then they anyway use log-transformed length and weight, so I'm a little confused by that. Anyway, I'll likely stick with the log-transformation!

# Model 1
m1 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm -1,  time_varying = ~ 1, data = d, time = "year",
             spde = spde, family = student(link = "identity", df = 2), ar1_fields = FALSE,
             include_spatial = TRUE, spatial_trend = FALSE, spatial_only = FALSE,
             silent = TRUE, newton_steps = 1, reml = TRUE)

# Model 2
m2 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm, data = d, time = "year",
             spde = spde, family = student(link = "identity", df = 2), ar1_fields = FALSE,
             include_spatial = TRUE, spatial_trend = TRUE, spatial_only = FALSE,
             silent = TRUE, newton_steps = 1, reml = TRUE)

m2b <- sdmTMB(formula = ln_weight_g ~ ln_length_cm-1, data = d, time = "year",
             spde = spde, family = student(link = "identity", df = 2), ar1_fields = FALSE,
             include_spatial = TRUE, spatial_trend = TRUE, spatial_only = FALSE,
             silent = TRUE, newton_steps = 1, reml = TRUE)

# Model 3
m3 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm, data = d, time = "year",
             spde = spde, family = student(link = "identity", df = 2), ar1_fields = TRUE,
             include_spatial = TRUE, spatial_trend = FALSE, spatial_only = FALSE,
             silent = TRUE, newton_steps = 1, reml = TRUE) 
...
ML: Great! At the moment I do have to run it for all (with the fewer knots I had in the beginning this only occured for some some models). Thanks!
...
df$residuals_m2b <- residuals(m2b)

qqnorm(df$residuals_m2b); abline(a = 0, b = 1)
...
ML: I tried plotting them against each other and they are not *exactly* on a 1:1 line, so there's some difference  
...
ML: Damn, I forgot about that! I didn't plan to change fixed effects, but ofc I did that later when adding the fixed year effect... I will sort this out and start with the random effects!
...
ML: That's a very interesting paper! I didn't realize it was like that... If I understand it correctly, I can't do the trick in the paper with removing the intercept, since I in addition have the categorical sex effect (and the -1 will then remove the female intercept). So, if I keep the sex-variable, I will calculate the correct standard 

```{r plot fixed effects on grid, fig.width=12}
spde <- make_mesh(data = d, xy_cols = c("lon", "lat"), n_knots = 5,
                 type = "kmeans", seed = 42)
m1 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm + year_f, data = d,
            spde = spde, family = student(link = "identity"), ar1_fields = FALSE,
            include_spatial = FALSE, spatial_trend = FALSE, spatial_only = TRUE,
            silent = TRUE)

m2 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm + year_f -1, data = d,
            spde = spde, family = student(link = "identity"), ar1_fields = FALSE,
            include_spatial = FALSE, spatial_trend = FALSE, spatial_only = TRUE,
            silent = TRUE)

m3 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm + sex_f + year_f -1, data = d,
            spde = spde, family = student(link = "identity"), ar1_fields = FALSE,
            include_spatial = FALSE, spatial_trend = FALSE, spatial_only = TRUE,
            silent = TRUE)

m4 <- sdmTMB(formula = ln_weight_g ~ ln_length_cm + year_f + sex_f -1, data = d,
            spde = spde, family = student(link = "identity"), ar1_fields = FALSE,
            include_spatial = FALSE, spatial_trend = FALSE, spatial_only = TRUE,
            silent = TRUE)

print(m1) # year 1991 is intercept, can't compare s.e's of reference and contrasts directly
print(m2) # no intercept, can compare years directly
print(m3) # can't compare, since 1991 is still intercept and other years are contrasts (since -1 removes the female intercept)
print(m4)

test <- sdmTMB(formula = ln_weight_g ~ ln_length_cm + year_f -1, data = d,
            spde = spde, family = student(link = "identity", df = 5))
```
