...
cpue_map_94_17 <- plot_map_raster_labels +
...
(ind_plot_sd | ind_plot_tot) / cpue_map_94_17 + plot_annotation(tag_levels = 'A')
...
depth_median <- wm_depth %>%
  filter(series %in% c("Density-weighted (5th decile, median)"))

depth_range <- wm_depth %>%
  mutate(series = as.factor(series)) %>% 
  filter(!series %in% c("Density-weighted (5th decile, median)")) %>% 
  dplyr::select(-depth_ct) %>% 
  pivot_wider(names_from = series, values_from = depth) %>% 
  rename("lwr10" = "Density-weighted (1st decile)",
         "upr90" = "Density-weighted (9th decile)")

plot_w_dep <- ggplot(depth_median, aes(year, depth, color = series, group = series, fill = series)) +
  geom_ribbon(data = depth_range, aes(x = year, ymin = lwr10, ymax = upr90),
...
# Without the ribbon
ggplot(depth_median, aes(year, depth, color = series, group = series, fill = series)) +
  stat_smooth(method = "gam", formula = y ~ s(x, k = 4), se = FALSE, size = 1) +
  geom_point(size = 2.5, alpha = 0.8, color = "white", shape = 21) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
  scale_color_manual(values = pal) +
...
# Quickly compare the mean:
# predict_mcod %>%
#   group_by(year, sub_div) %>%
#   summarise(density_weighted_mean = weighted.mean(x = oxy, w = exp(est))) %>% 
#   as.data.frame()

oxygen_series <- bind_rows(wm_oxy, annual_oxy)

oxygen_median <- oxygen_series %>%
  mutate(series = as.factor(series)) %>% 
  filter(series %in% c("Median in environment", "Density-weighted (5th decile, median)"))

oxygen_range <- oxygen_series %>%
  mutate(series = as.factor(series)) %>% 
  filter(!series %in% c("Median in environment", "Density-weighted (5th decile, median)")) %>% 
  dplyr::select(-oxy_ct) %>% 
  pivot_wider(names_from = series, values_from = oxy) %>% 
  rename("lwr10" = "Density-weighted (1st decile)",
         "upr90" = "Density-weighted (9th decile)")

plot_w_oxy <-
ggplot(oxygen_median, aes(year, oxy, color = series, group = series, fill = series, linetype = series)) +
  geom_ribbon(data = oxygen_range, aes(x = year, ymin = lwr10, ymax = upr90),
              inherit.aes = FALSE, fill = "grey90") +
  stat_smooth(method = "gam", formula = y ~ s(x, k = 4), se = FALSE, size = 1) +
  geom_point(size = 2.5, alpha = 0.8, color = "white", shape = 21) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
...
### Weighted oxygen by year and sub div
```{r weighted oxygen by subdiv and year}
...
wm_temp <- predict_mcod %>%
  group_by(year) %>%
  summarise("Density-weighted (5th decile, median)" = hutils::weighted_quantile(v = temp, w = exp(est), p = c(0.5)),
            "Density-weighted (1st decile)" = hutils::weighted_quantile(v = temp, w = exp(est), p = c(0.1)),
            "Density-weighted (9th decile)" = hutils::weighted_quantile(v = temp, w = exp(est), p = c(0.9))) %>%
  pivot_longer(cols = c("Density-weighted (5th decile, median)", "Density-weighted (1st decile)", "Density-weighted (9th decile)"),
...
temp_median <- temp_series %>%
  mutate(series = as.factor(series)) %>% 
  filter(series %in% c("Median in environment", "Density-weighted (5th decile, median)"))

temp_range <- temp_series %>%
  mutate(series = as.factor(series)) %>% 
  filter(!series %in% c("Median in environment", "Density-weighted (5th decile, median)")) %>% 
  dplyr::select(-temp_ct) %>% 
  pivot_wider(names_from = series, values_from = temp) %>% 
  rename("lwr10" = "Density-weighted (1st decile)",
         "upr90" = "Density-weighted (9th decile)")

plot_w_temp <- ggplot(temp_median, aes(year, temp, color = series, group = series, fill = series, linetype = series)) +
  geom_ribbon(data = temp_range, aes(x = year, ymin = lwr10, ymax = upr90),
              inherit.aes = FALSE, fill = "grey90") +
  stat_smooth(method = "gam", formula = y ~ s(x, k = 4), se = FALSE, size = 1) +
  geom_point(size = 2.5, alpha = 0.8, color = "white", shape = 21) +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
