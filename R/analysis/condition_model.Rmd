...
pred_grid2 <- pred_grid2 %>%
  mutate(ln_length_cm = 0) %>% # For now we'll predict changes in the intercept ("condition factor"), so ln_length_cm = 0 (log(1))
  mutate(year = as.integer(year)) %>% 
...
plot(oxy ~ depth, data = pred_grid2)
m1 <- lm(oxy ~ depth, data = pred_grid2)
nd <- data.frame(depth = c(40, 45))
nd$pred <- predict(m1, newdata = nd)
nd
nd$pred[2] - nd$pred[1]

plot(temp ~ depth, data = pred_grid2)
m1 <- lm(temp ~ depth, data = filter(pred_grid2, depth < 100))
nd <- data.frame(depth = c(40, 45))
nd$pred <- predict(m1, newdata = nd)
nd
nd$pred[2] - nd$pred[1]
...
pred_grid2 %>% 
  ...
  ungroup() %>% 
  ggplot(., aes(year, mean_oxy, color = subdiv)) +
  geom_line()
```
...
cond_slope_fig <- cond_year_slopes %>% 
  ...
  geom_hline(yintercept = 0, alpha = 0.5, linetype = 2, color = "red", alpha = 0.5) +
...
d %>% 
  ...
  ggplot(., aes(year, le_cren, color = factor(Deciles), fill = factor(Deciles))) +
  stat_smooth() + 
...
ggsave("figures/supp/condition/le_cren_quantiles.png", width = 6.5, height = 6.5, dpi = 600)
```
...
ggsave("figures/supp/condition/all_years_condition_map.png", width = 6.5, height = 6.5, dpi = 600)
