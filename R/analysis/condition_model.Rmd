...
median(d$weight_g)
# What does an effect size of 0.0034 mean for change in log weight and weight?
t <- tibble(l = seq(1, 80, 0.1)) %>% 
  mutate(w1 = 0.01*l^3,
         log_w1 = log(w1),
         log_w2 = log_w1*1.0034,
         w2 = exp(log_w2),
         perc_chang_log_w = ((log_w2-log_w1)/log_w1)*100,
         perc_chang_w = ((w2-w1)/w1)*100) %>% 
  as.data.frame()

t %>% filter(w1 > median(0.95*d$weight_g)) %>% head(10)

ggplot(t) +
  geom_line(aes(l, w1, color = "w1")) +
  geom_line(aes(l, w2, color = "w2"))

ggplot(t) +
  geom_line(aes(w1, perc_chang_w, color = "w1"))

# Now calculate the change in weight given a decline in the intercept
t2 <- tibble(l = seq(1, 80, 0.1)) %>% 
  mutate(w1 = exp(-4.5)*l^2.975,
         w2 = exp(-4.66)*l^2.975) %>% 
  as.data.frame()
...
p1 <- ggplot(t2) + 
  geom_line(aes(l, w1, color = "1993"), size = 1.2, alpha = 0.8) +
  geom_line(aes(l, w2, color = "2008"), size = 1.2, alpha = 0.8, linetype = 2) + 
...
p2 <- ggplot(t2) + 
  geom_line(aes(log(l), log(w1), color = "1993"), size = 1.2, alpha = 0.8) +
  geom_line(aes(log(l), log(w2), color = "2008"), size = 1.2, alpha = 0.8, linetype = 2) + 
  scale_color_manual(values = pal) + 
  labs(color = "Year", x = "Length [cm]", y = "Weight [g]") +
  theme_light(base_size = 16) + 
  theme(axis.text.x = element_text(angle = 0),
...
ggsave("figures/lw_relationship.png", width = 6.5, height = 6.5, dpi = 600)
...
## Difference in weight-at-length using sims!
...
# 
# ggplot(lw_pred_df_long, aes(length_cm, weight_g, group = iteration, color = factor(year))) + 
