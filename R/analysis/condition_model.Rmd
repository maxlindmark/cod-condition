...
# Scale the variables with respect to data!
data_means <- d %>%
  dplyr::select(biomass_her, biomass_her_sd, biomass_spr, biomass_spr_sd,
                density_cod, density_cod_rec, #density_cod_sd,
                density_fle, density_fle_rec, #density_fle_sd,
                density_saduria, density_saduria_rec, #density_saduria_sd,
...
pred_grid <- pred_grid %>%
  mutate(ln_length_cm = log(1),
         ...
         #density_cod_sd_sc = (density_cod_sd - data_means$density_cod_sd) / data_stdev$density_cod_sd,
         ...
         #density_fle_sd_sc = (density_fle_sd - data_means$density_fle_sd) / data_stdev$density_fle_sd,
         ...
         #density_saduria_sd_sc = (density_saduria_sd - data_means$density_saduria_sd) / data_stdev$density_saduria_sd,
         depth_sc = (depth - data_means$depth) / data_stdev$depth,
         depth_rec_sc = (depth_rec - data_means$depth_rec) / data_stdev$depth_rec,
         #depth_sd_sc = (depth_sd - data_means$depth_sd) / data_stdev$depth_sd,
         oxy_sc = (oxy - data_means$oxy) / data_stdev$oxy,
         oxy_rec_sc = (oxy_rec - data_means$oxy_rec) / data_stdev$oxy_rec,
         #oxy_sd_sc = (oxy_sd - data_means$oxy_sd) / data_stdev$oxy_sd,
         temp_sc = (temp - data_means$temp) / data_stdev$temp,
         temp_rec_sc = (temp_rec - data_means$temp_rec) / data_stdev$temp_rec,
         #temp_sd_sc = (temp_sd - data_means$temp_sd) / data_stdev$temp_sd
         )

# Plot on map:
ggplot(pred_grid, aes(X, Y)) + geom_point(size = 0.1) + facet_wrap(~year)
...
# Replace the ices-rectangle pelagic values that are NA with the mean in the sub division
# Replace the sub-division pelagic values that are NA with the mean in the year
# Since this is the scaled variable, we just set them to zero
pred_grid <- pred_grid %>% 
  mutate(biomass_spr_sc = replace_na(biomass_spr_sc, 0),
         biomass_her_sc = replace_na(biomass_her_sc, 0),
         biomass_spr_sd_sc = replace_na(biomass_spr_sd_sc, 0),
         biomass_her_sd_sc = replace_na(biomass_her_sd_sc, 0))
...
## Difference in intercept (log condition factor) between the end and start of time period using sims

```{r simulate intercepts,cache=TRUE}
colnames(pred_grid)

# Get a dataframe with all covariates at 0 but all years
# z <- rep(0, length(c(1993:2019)))
# pred_intercept_df <- data.frame(year = c(1993:2019), ln_length_cm = z, biomass_her_sc = z,
#                                 biomass_her_sd_sc = z, biomass_spr_sc = z, biomass_spr_sd_sc = z,
#                                 density_cod_sc = z, density_cod_rec_sc = z, density_fle_sc = z,
#                                 density_fle_rec_sc = z, density_saduria_sc = z, density_saduria_rec_sc = z,
#                                 depth_sc = z, depth_rec_sc = z, oxy_sc = z, oxy_rec_sc = z, temp_sc = z, temp_rec_sc = z)

# Predict with 500 sims
# pred_intercept <- predict(mfull, re_form = ~0, re_form_iid = NULL,
#                           newdata = pred_intercept_df, sims = 500)
...
```{r test condition factor vs alpha}
# d_alpha <- data.frame(mfull$sd_report$par.random[1:27])
# 
# avg_pred$alpha <- d_alpha$mfull.sd_report.par.random.1.27
# 
# avg_pred_long <- avg_pred %>% dplyr::select(year, est, alpha, lwr, upr) %>% 
#   pivot_longer(2:3)
# 
# ggplot(avg_pred_long, aes(year, value, color = name)) +
...
cond_index <- index_avg_sim %>%
  ggplot(., aes(year, log(est))) +
  #scale_x_continuous(breaks = scales::pretty_breaks(n = 10)) +
  labs(y = "log(condition factor)", x = "Year") +
  geom_line(data = div_index, aes(year, log(est), color = factor(sub_div)), alpha = 0.6) + 
  scale_color_brewer(palette = "Dark2", name = "ICES\nSub division") +
  geom_point(size = 2.3, shape = 21, fill = "gray10", color = "white") +
  geom_errorbar(aes(x = year, ymax = log(upr), ymin = log(lwr)),
                width = 0.6, alpha = 0.8, color = "gray10", size = 0.5) +
...
# p <- 
#   ggplot(swe_coast_proj) +
#   geom_raster(data = pred_map, aes(x = X*1000, y = Y*1000, fill = est)) +
#   geom_sf(size = 0.3) +
#   scale_fill_gradientn(colours = rev(pal), name = "log(condition factor)") + 
#   labs(x = "Longitude", y = "Latitude")
# 
...
# Prepare prediction data frame
nd_oxy <- data.frame(oxy_rec_sc = seq(min(d$oxy_rec_sc), max(d$oxy_rec_sc), length.out = 100))
...
mar_oxy <- ggplot(p_margin_oxy, aes(oxy_rec_sc, est, ymin = est - 1.96 * est_se, ymax = est + 1.96 * est_se)) +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  #coord_cartesian(ylim = c(-4.65, -4.4)) + 
  theme(aspect.ratio = 1) + 
  labs(x = expression(Oxygen[rec]))
...
# Prepare prediction data frame
nd_tem <- data.frame(temp_rec_sc = seq(min(d$temp_rec_sc), max(d$temp_rec_sc), length.out = 100))
...
mar_temp <- ggplot(p_margin_tem, aes(temp_rec_sc, est, ymin = est - 1.96 * est_se, ymax = est + 1.96 * est_se)) +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  #coord_cartesian(ylim = c(-4.65, -4.4)) + 
  theme(aspect.ratio = 1) + 
  labs(x = expression(Temperature[rec]))
```
...
# Prepare prediction data frame
nd_cod <- data.frame(density_cod_rec_sc = seq(min(d$density_cod_rec_sc), max(d$density_cod_rec_sc), length.out = 100))
...
mar_cod <- ggplot(p_margin_cod, aes(density_cod_rec_sc, est, ymin = est - 1.96 * est_se, ymax = est + 1.96 * est_se)) +
  geom_ribbon(alpha = 0.4) +
  geom_line() +
  #coord_cartesian(ylim = c(-4.65, -4.4)) + 
  theme(aspect.ratio = 1) + 
