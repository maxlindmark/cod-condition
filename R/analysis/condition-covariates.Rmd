---
title: "Condition models with covariates"
author: "Max Lindmark"
date: "9/18/2020"
output:
  html_document:
...
# Calculate some variables in data (I will next standardize the covariates to have
# a mean of 0 and variance of 1 to facilitate comparison between different ones, and
# read in the prediction grid.
d <- d %>%
  mutate(ln_weight_g = log(weight_g),
         ln_length_cm = log(length_cm),
         Fulton_K = weight_g/(0.00692*length_cm^3.08)) %>% # Changed to cod-specific age-length parameters
...
spde <- make_spde(d$lon, d$lat, n_knots = 60)
plot_spde(spde)
...
```{r spatiotemporal glmm model fit, results='hide', message=FALSE}
...
2 * plogis(example_est$ar1_phi) - 1
...
ML: Those are good ideas actually. 
