...
## Fit models
### First fit a non spatial model to get the average length weight relationship. The main model's response variable is the difference in observed vs this predicted average weight.
...
### Fit the condition model without covariates

```{r fit spatial model without covariates to see how it compares, cache=TRUE}
ptm <- proc.time()
mnull_spatial <- sdmTMB(
  formula = le_cren ~ -1 + year_f,
  time_varying = NULL,
  data = d,
  time = "year",
  mesh = spde,
...
ptm <- proc.time()
mfull <- sdmTMB(
  formula = le_cren ~ -1 + year_f + biomass_her_sc + biomass_her_sd_sc +
...
```{r spatiotemporal le cren model with breakpoint oxygen, cache=TRUE}
ptm <- proc.time()
mfull_threshold <- sdmTMB(
  formula = le_cren ~ -1 + year_f + biomass_her_sc + biomass_her_sd_sc +
    biomass_spr_sc + biomass_spr_sd_sc +
    density_cod_sc + density_cod_rec_sc + 
    density_fle_sc + density_fle_rec_sc + 
    density_saduria_sc + density_saduria_rec_sc + 
    depth_sc + depth_rec_sc + 
    breakpt(oxy_sc) + oxy_rec_sc + # breakpoint model
    temp_sc + temp_rec_sc,
    data = d,
  time = "year",
  mesh = spde, 
...
p_bp <- predict(mfull_threshold, newdata = nd_bp, se_fit = TRUE, re_form = NA, xy_cols = c("X", "Y"))

ggplot(p_bp, aes(oxy_sc, est, 
...
```{r compare aic}
AIC(mfull, mfull_threshold)
```

```{r check model convergence, cache=TRUE}
# Check model convergence
sanity(mfull)
```
...
b_fe2 <- b_fe2[28:43] # skip year coefficients
