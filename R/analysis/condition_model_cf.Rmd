...
```{r forst fit non_spatial model without covariates, cache=TRUE}
ptm <- proc.time()
mnull <- sdmTMB(
...
```{r forst fit spatial model without covariates to see how it compares, cache=TRUE}
ptm <- proc.time()
mnull_spatial <- sdmTMB(
  formula = ln_weight_g ~ ln_length_cm,
  time_varying = NULL, data = d, time = "year", mesh = spde,
  family = student(link = "identity", df = 5), 
...
# Extract average a and b across the total dataset
# a <- tidy(mnull_spatial, conf.int = TRUE)[1, "estimate"]
# b <- tidy(mnull_spatial)[2, "estimate"]
```
...
ptm <- proc.time()
mfull <- sdmTMB(
  formula = le_cren ~ -1 + biomass_her_sc + biomass_her_sd_sc +
...
# Marginal R2s
denominator <- VarF + VarO + VarE + VarV + VarR

marg <- VarF/denominator

out <- data.frame(
    marginal = marg,  
    partial_time_varying = VarV/denominator,
    partial_spatial = VarO/denominator,  
    partial_spatiotemporal = VarE/denominator,
    condional = (VarF + VarO + VarE + VarV)/denominator,
    all_random = (VarO + VarE + VarV)/denominator,
    marginal_random_ratio = marg / ((VarO + VarE + VarV)/denominator),
    random_marginal_ratio = ((VarO + VarE + VarV)/denominator) / marg
  ) 
...
index_avg_sims %>% 
  ...
  ggplot(., aes(as.factor(year), log(.value))) +
...
# Here are the sims for each year. Take the deciles of these
# ggplot(index_avg_sims, aes(y = as.factor(year), x = log(.value))) + 
#   geom_density_ridges_gradient(rel_min_height = 0.01, alpha = 0.8, color = "grey10", size = 0.2,
#                                quantile_lines = TRUE, quantiles = 0.5) + 
#   coord_flip()
...
# deciles <- ggplot() +
#   stat_smooth(data = deciles_sum, aes(year, le_cren, color = Deciles), method = "gam", formula = y~s(x, k = 3),
#               se = TRUE, show.legend = FALSE) + 
#   geom_point(data = deciles_sum, aes(year, le_cren, color = Deciles)) +
...
write.csv(div_index_delta_cond, "div_index_delta_cond.csv")
...
ggplot(d, aes(depth)) + 
  geom_density() + 
  geom_vline(xintercept = c(5, 120)) +
  coord_cartesian(expand = 0)

plot_map_raster +
  geom_raster(data = filter(pred_map, depth < 120 & depth > 10), aes(x = X*1000, y = Y*1000, fill = est)) +
