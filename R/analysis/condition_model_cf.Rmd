...
```{r check model convergence spatial null model}
...
```{r check model convergence bp model}
# Check model convergence
sanity(mfull_threshold)
```
...
data_ridge <- ggplot(d, aes(x = le_cren, y = fct_rev(as.factor(year)), fill = factor(stat(quantile)))) +
  ...
  theme(axis.title = element_text(size = 9),
        ...
        plot.margin = unit(c(0.4, 0.67, 0.4, 0), "cm")) +
  NULL
...
```{r testing effect of rw or factor year for intercept estimates in a simple model}
