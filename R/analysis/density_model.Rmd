...
mcmc_res <- residuals(m, type = "mle-mcmc", mcmc_iter = 5000, mcmc_warmup = 2000,
...
l <- (bathy_plot / oxy_plot / temp_plot) & theme(legend.text = element_text(size = 5),
                                                 legend.title = element_text(size = 5))

r <- (plot_w_dep / plot_w_oxy / plot_w_temp) & theme(legend.text = element_text(size = 6),
                                                     legend.title = element_text(size = 7))
...
ggsave("figures/weighted_quantiles.pdf", width = 14, height = 17, units = "cm")
```
...
mean_oxy_temp %>% 
  filter(variable == "Temperature",
         !year == 1994) %>% 
  mutate(species2 = ifelse(species == "Environment", 2, 1)) %>% 
  ggplot(aes(year, value,
             color = reorder(species, species2),
             linetype = reorder(species, species2),
             size = reorder(species, species2))) + 
  geom_point(size = 2.3, alpha = 0.5) +
  guides(size = "none", linetype = "none") +
  scale_linetype_manual(values = c(1,1,1,1,2)) +
  scale_size_manual(values = c(1,1,1,1,2.3)) +
...
ggsave("figures/supp/density/weighted_mean_temp_species.png", width = 6.5, height = 6.5, dpi = 600)
...
#   scale_color_viridis() + 
#   theme(plot.margin = unit(c(0, 0.1, 0, 0.2), "cm"),
