...
d$depth_agg <- raster::extract(dep_rast2_rect_agg, d %>% dplyr::select(lon, lat), method = "bilinear")
...
saduria <- terra::rast("data/saduria_tif/FWBiomassm_raster_19812019presHighweightcor_no0_newZi.tif")

newcrs <- "+proj=longlat +datum=WGS84"

saduria_longlat = terra::project(saduria, newcrs)
...
d$density_saduria_agg <- raster::extract(sad2_rect_agg, d %>% dplyr::select(lon, lat))$FWBiomassm_raster_19812019presHighweightcor_no0_newZi
...
index_keep_q4 <- which(months %in% c(10, 11, 12))
...
for(i in loop_seq_q4) { # We can use q4 as looping index, doesn't matter!
...
for(i in unique(d_sub_oxy$year_f)) {
  ...
  # Aggregate
  res(r_q4) # degrees, roughly 3000 m
  
  fact_rec = (55000/2) / 3000
  
  r_q4_agg <- terra::aggregate(r_q4, fact = fact_rec, fun = mean)
  ...
  rasValue_q4 <- raster::extract(r_q4_agg, data_sp, method = "bilinear")
  
  # Add in the raster value in the df holding the coordinates for the pred-grid data
  # Q4 is our main value, but we also want q4 average
  d_slice$oxy_q4 <- rasValue_q4
...
d$oxy_q4 <- d$oxy_q4 * 0.0223909

# Compare with ices_rect
ggplot(d, aes(oxy_q4, oxy_rec)) + 
...
# Now we have an array with Q4 data...
# We need to now calculate the average within a year.
# Get a sequence that takes every third value between 1: number of months (length)
